...
const Stories = () => {
  ...
  const toggleFavorite = (storyId) => {
    setFavorites(prevFavorites => ({
      ...prevFavorites,
      [storyId]: !prevFavorites[storyId]
    }));
    setMessage(prevMessage => ({
      ...prevMessage,
      [storyId]: !favorites[storyId] ? 'Added to favorites' : 'Removed from favorites'
    }));
    setTimeout(() => {
      setMessage(prevMessage => ({
        ...prevMessage,
  ...
  return (
    <div className="container mx-auto p-4 text-white rounded-lg">
      <h1 className="text-2xl font-bold mb-4">Stories in {categoryName}</h1>
